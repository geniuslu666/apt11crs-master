...
import { AgreeChannel, TransferChannel, ChannelList } from '@/api/pmsWithdraw';
    ...
    return h(TableAction as any, {
      ...
      actions: [
        ...
        },
        {
          ...
          auth: ['/pmsWithdraw/disagreeChannel'],
          type: 'warning'
        },
        {
          label: '转账',
          ...
          auth: ['/pmsWithdraw/transferChannel'],
          type: 'info'
        },
        {
          label: '详情',
...
const loadDataTable = async (res) => {
  return await ChannelList({ ...searchFormRef.value?.formModel, ...res });
};
...
function handleAgree(record: Recordable) {
  ...
  dialog.info({
    ...
    negativeText: '取消',
    onPositiveClick: () => {
      AgreeChannel({id: record.id}).then((_res) => {
...
function handleTransfer(record: Recordable) {
  ...
  dialog.info({
    ...
    negativeText: '取消',
    onPositiveClick: () => {
      TransferChannel({id: record.id}).then((_res) => {
        message.success('操作成功');
        reloadTable();
      });
    },
