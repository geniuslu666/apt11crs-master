<template>
  <div>
    <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
      <template #tableTitle>
        <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/foodSettlementAccount/edit'])">
          <template #icon>
            <n-icon>
              <PlusOutlined />
            </n-icon>
        ...
        <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/foodSettlementAccount/delete'])">
          <template #icon>
            <n-icon>
              <DeleteOutlined />
            </n-icon>
...
import { List, Delete, Status } from '@/api/foodSettlementAccount';
      ...
      actions: [
        {
          ...
          auth: ['/foodSettlementAccount/edit'],
        },

        {
          ...
          auth: ['/foodSettlementAccount/status'],
        },
        {
          label: '启用',
        ...
        },
        {
          ...
          auth: ['/foodSettlementAccount/delete'],
        },
      ],
    });
  },
