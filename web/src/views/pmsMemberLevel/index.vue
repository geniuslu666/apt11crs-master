  ...
  <div>
    <div class="n-layout-page-header" style="margin: 0">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">会员等级</text>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
<!--          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/pmsMemberLevel/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
...
<!--          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/pmsMemberLevel/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
import {h, reactive, ref, computed, onMounted} from 'vue';
  ...
  import { List, Delete, Status } from '@/api/pmsMemberLevel';
  ...
  import Edit from "@/views/pmsMemberLevel/edit.vue";
  import View from "@/views/pmsMemberLevel/view.vue";
      ...
      return h(TableAction as any, {
        ...
        actions: [
          ...
          },*/
          {
            ...
            auth: ['/pmsMemberLevel/view'],
          },
          {
            label: '编辑',
            onClick: handleEdit.bind(null, record),
            auth: ['/pmsMemberLevel/edit'],
          },

          // {
          //   label: '删除',
          //   onClick: handleDelete.bind(null, record),
          //   auth: ['/pmsMemberLevel/delete'],
          // },
        ],
...
/*
  function addTable() {
    ...
    router.push({ name: 'pmsMemberLevel_edit', params: { id: null } });
  }
*/
  ...
  function handleEdit(record: Recordable) {
    ...
    // router.push({ name: 'pmsMemberLevel_edit', params: { id: record.id } });
  }
