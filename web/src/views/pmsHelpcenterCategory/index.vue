  ...
  <div>
    <div class="n-layout-page-header" style="margin: 0">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">帮助中心分类</text>
        </template>
...
<!--      </BasicForm>-->
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/pmsHelpcenterCategory/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/pmsHelpcenterCategory/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
import { List, Delete } from '@/api/pmsHelpcenterCategory';
      ...
      actions: [
        {
          ...
          auth: ['/pmsHelpcenterCategory/edit'],
        },

        {
          label: '删除',
          onClick: handleDelete.bind(null, record),
          auth: ['/pmsHelpcenterCategory/delete'],
        },

        {
          label: '详情',
          onClick: handleView.bind(null, record),
          auth: ['/pmsHelpcenterCategory/view'],
        },
      ],
    });
  },
