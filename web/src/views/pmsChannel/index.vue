  ...
  <div>
      ...
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <n-divider title-placement="left" style="margin: 0">
            <span style="font-size: 18px;color: #3D3D3D;line-height: 25px;font-weight: 500">渠道管理</span>
          </n-divider>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/pmsChannel/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/pmsChannel/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
import {List, Delete, Status, Unbind} from '@/api/pmsChannel';
...
import View from '@/views/pmsChannel/view.vue';
    ...
    return h(TableAction as any, {
      ...
      actions: [
        ...
        },
        {
          ...
          auth: ['/pmsChannel/unbind'],
          type: 'warning'
        },
        {
          label: '详情',
          onClick: handleView.bind(null, record),
          auth: ['/pmsChannel/view'],
        },
      ],
    });
  },
...
function handleView(record: Recordable) {
  ...
  // router.push({ name: 'pmsChannel_view', params: { id: record.id } });
}
