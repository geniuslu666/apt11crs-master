...
import { Bind } from '@/api/pmsChannel';
...
function openModal(channelId, memberId = 0) {
  initChannelId.value = channelId;
...
function handleChoose(record: Recordable) {
  loading.value = true
  Bind({
    id: initChannelId.value,
    pmsMemberId: record.id
  }).then((_res) => {
