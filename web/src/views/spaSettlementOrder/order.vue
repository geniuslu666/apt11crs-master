  ...
  <div>
    <div class="n-layout-page-header">
      <n-spin :show="show" description="请稍候...">
        <n-card :bordered="false" :title="technicianName+ ' - 订单结算'">
...
import { TechnicianOrderList } from '@/api/spaOrder';
import { columns, schemas } from './order';
import { adaTableScrollX, getOptionLabel } from '@/utils/hotgo';
import {useRouter} from "vue-router";
import {View as SettlementOrderView} from "@/api/spaSettlementOrder";
...
const loadDataTable = async (res) => {
  return await TechnicianOrderList({ ...searchFormRef.value?.formModel, ...res, settlementOrderId: params.id });
};
...
async function loadSettlementView(){
  ...
  if (info.settlementObject == "ISP") {
    technicianName.value = info.ispDetail.name
  }else{
    technicianName.value = info.technicianDetail.name
  }
