  ...
  <div>
    <n-drawer v-model:show="showModal" :width="dialogWidth">
      <n-drawer-content closable :header-style="{
        ...
        <template #footer v-if="!loading && formValue.orderStatus != 'WAIT_PAY' && formValue.orderStatus != 'CANCEL'">
          <template v-if="formValue.abnormalStatus < 2">
            <template v-if="formValue.orderStatus == 'WAIT_CONFIRM'">
              <n-button type="primary" v-if="hasPermission(['/carOrder/confirmAgree'])" @click="agreeOrder"
                :loading="formBtnLoading1" style="width: 70px;height: 35px;margin-right: 10px">
                确认
              </n-button>
              <n-button type="warning" v-if="hasPermission(['/carOrder/confirmDisagree'])" @click="disAgreeOrder"
                style="width: 70px;height: 35px;margin-right: 10px">
                拒绝
              </n-button>
            </template>
            <template
              v-if="formValue.orderType == 'CRS' && formValue.orderStatus == 'WAIT_SERVE' && formValue.dispatchStatus == 'WAIT'">
              <n-button type="primary" v-if="hasPermission(['/carOrder/dispatch'])" @click="dispatchOrder"
                style="width: 98px;height: 35px;margin-right: 10px">
                指派司机
              </n-button>
            </template>
            <template
              v-if="formValue.orderType == 'CRS' && formValue.orderStatus == 'WAIT_SERVE' && formValue.dispatchStatus == 'DONE'">
              <n-button type="primary" v-if="hasPermission(['/carOrder/transfer'])" @click="transferOrder"
                style="width: 98px;height: 35px;margin-right: 10px">
                转单
              </n-button>
            </template>
            <template
              v-if="formValue.orderType == 'CRS' && formValue.orderStatus == 'WAIT_SERVE' && formValue.dispatchStatus == 'DONE' && !formValue.driverGoTime">
              <n-button type="primary" v-if="hasPermission(['/carOrder/goOut'])" @click="goOutOrder"
                :loading="formBtnLoading" style="width: 98px;height: 35px;margin-right: 10px">
                司机出发
              </n-button>
            </template>
            <template
              v-if="formValue.orderType == 'CRS' && formValue.orderStatus == 'WAIT_SERVE' && formValue.dispatchStatus == 'DONE' && formValue.driverGoTime">
              <n-button type="primary" v-if="hasPermission(['/carOrder/startService'])" @click="startServerOrder"
                :loading="formBtnLoading" style="width: 98px;height: 35px;margin-right: 10px">
                开始服务
              </n-button>
            </template>
            <template v-if="formValue.orderType == 'CRS' && formValue.orderStatus == 'SERVING'">
              <n-button type="primary" v-if="hasPermission(['/carOrder/endService'])" @click="endServerOrder"
                :loading="formBtnLoading" style="width: 98px;height: 35px;margin-right: 10px">
                结束服务
              </n-button>
            </template>
          ...
          <template v-else>
            <template v-if="formValue.abnormalStatus == 2">
              <n-button type="error" @click="abnormalOrder" v-if="hasPermission(['/carOrder/abnormal'])"
                style="width: 98px;height: 35px;margin-right: 10px">
                异常处理
              </n-button>
            </template>
          ...
          <template
            v-if="formValue.canRefund">
            <n-button type="error" v-if="hasPermission(['/carOrder/refund'])" @click="refundOrder"
                      style="width: 70px;height: 35px;margin-right: 10px">
              退款
            </n-button>
          </template>
          <n-button v-if="hasPermission(['/printer/printCarOrder'])" @click="printOrder" :loading="formBtnLoading4"
        ...
        <n-spin :show="loading" description="请稍候...">
          <div style="padding: 0 20px">
            <div class="car-order-detail-top">
              {{ formValue.serviceType == 'PICKUP' ? '接机' : '送机' }}服务
              <div class="car-order-detail-top-type" :class="formValue.orderType == 'CRS' ? 'crs' : 'innn'">{{
                formValue.orderType == 'CRS' ? '自营' : 'INNN' }}</div>
              <n-tag size="small" :bordered="false" type="error" style="margin-left: 10px"
                v-if="formValue.abnormalStatus == 2">
                异常待处理
              </n-tag>
            ...
            <div class="car-order-detail-address flex-col !items-start gap-2 md:flex-row md:!items-center">
              <div class="car-order-detail-address-item">
                <div class="car-order-detail-address-item-d1">出发</div>
                <div class="car-order-detail-address-item-d2">{{ formValue.startServiceAddress.name }}</div>
                <n-tooltip trigger="hover" :arrow-point-to-center="true">
                  <template #trigger>
                    <div class="car-order-detail-address-item-d3">
                      {{ formValue.startServiceAddress.detailAddress }}
                    </div>
                  </template>
                  {{ formValue.startServiceAddress.detailAddress }}
                </n-tooltip>
              ...
              <div class="car-order-detail-address-item">
                <div class="car-order-detail-address-item-d1">到达</div>
                <div class="car-order-detail-address-item-d2">{{ formValue.endServiceAddress.name }}</div>
                <n-tooltip trigger="hover" :arrow-point-to-center="true">
                  <template #trigger>
                    <div class="car-order-detail-address-item-d3">
                      {{ formValue.endServiceAddress.detailAddress }}
                    </div>
                  </template>
                  {{ formValue.endServiceAddress.detailAddress }}
                </n-tooltip>
            ...
            <div class="car-order-detail-time flex-col !items-start gap-5 md:gap-1 md:flex-row md:!items-center">
              <div class="car-order-detail-time-item">
                <div class="car-order-detail-time-item-d1">用车时间</div>
              ...
              <div class="car-order-detail-time-item">
                ...
                <div class="car-order-detail-time-item-status"
                  :class="getOptionTag(options.spa_order_status, formValue.orderStatus)">{{
                    getOptionLabel(options.spa_order_status, formValue.orderStatus) }}</div>
              </div>
            </div>
            <div class="car-order-detail-time">
              <div class="car-order-detail-time-item">
                ...
                <div class="car-order-detail-time-item-d2">
                  <div>{{ formValue.memberDetail ? formValue.memberDetail.fullName : '-' }} / {{ formValue.memberDetail
                    ?
                    formValue.memberDetail.memberNo : '-' }}</div>
          ...
          <div style="padding: 30px 20px" v-if="nowTab == 1">
              ...
              <n-grid y-gap="25" :cols="2">
                ...
                <n-gi>
                  <div class="level-detail-div-item">
                    ...
                    <div class="level-detail-div-item-content">
                      <div>{{ formValue.adultNum }}成人</div>
                      <div>{{ formValue.childNum }}儿童</div>
                    </div>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">紧急联系人</div>
                    <div class="level-detail-div-item-content">
                      <template v-if="!formValue.emergencyName && !formValue.emergencyMobile">
                        --
                      </template>
                      <template v-else>
                        <div>{{ formValue.emergencyName ? formValue.emergencyName : '--' }}</div>
                        <div>{{ formValue.emergencyMobile ? formValue.emergencyPhoneArea + ' ' +
                          formValue.emergencyMobile : '--' }}</div>
                      </template>
                    </div>
                  </div>
                </n-gi>
                ...
                <n-gi span="2">
                  <div class="level-detail-div-item">
                    ...
                    <div class="level-detail-div-item-content" v-if="translateTab == 'zh'">{{ formValue.memberMessage ?
                      formValue.memberMessage : '--' }}</div>
                    <div class="level-detail-div-item-content" v-else>{{ formValue.memberMessageJa ?
                      formValue.memberMessageJa :
                      '--' }}</div>
                  </div>
                </n-gi>
              </n-grid>
            ...
            <div class="level-detail-div">
              ...
              <n-grid y-gap="25" :cols="2">
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">出发地</div>
                    <div class="level-detail-div-item-content">{{ formValue.startServiceAddress.name }}</div>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">目的地</div>
                    <div class="level-detail-div-item-content">{{ formValue.endServiceAddress.name }}</div>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">车型</div>
                    <div class="level-detail-div-item-content">{{ formValue.serviceDetail?.carTypeDetail?.name || '--'
                    }}</div>
                  </div>
                </n-gi>
                <n-gi>
                ...
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">附加服务</div>
                    <div class="level-detail-div-item-content">
                      <template v-if="formValue.orderType == 'CRS'">
                        <div v-if="formValue.pickUpSign == 'N' && formValue.childSeatAddNum <= 0">无</div>
                        <template v-else>
                          <div v-if="formValue.pickUpSign == 'Y'">举牌接机</div>
                          <div v-if="formValue.childSeatAddNum > 0">婴儿座椅 * {{ formValue.childSeatAddNum }}张</div>
                        </template>
                      </template>
                      <template v-else>
                        <div v-if="formValue.innnLuggageNum <= 0">无</div>
                        <template v-else>
                          <div v-if="formValue.innnLuggageNum > 0">额外行李 * {{ formValue.innnLuggageNum }}个</div>
                        </template>
                      </template>
                    </div>
                  </div>
                ...
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">航班号</div>
                    <div class="level-detail-div-item-content">{{ formValue.flightNumber ? formValue.flightNumber : '--'
                    }}
                    </div>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">司机</div>
                    <div class="level-detail-div-item-content">{{ formValue.driverDetail?.nickname || '--' }}</div>
                    <div class="level-detail-div-item-content" v-if="formValue.driverId > 0 && formValue.driverDeleted" style="color:red;margin-top: 5px">司机已删除</div>
                  </div>
                </n-gi>
                <n-gi span="2">
                  <div class="level-detail-div-item">
                    <div class="level-detail-div-item-title">指派司机/订单调度备注</div>
                    <div class="level-detail-div-item-content">{{ formValue.dispatchStatus == 'DONE' ?
                      (formValue.dispatchDesc ?
                        formValue.dispatchDesc : '无') : '--' }}</div>
                  </div>
                </n-gi>
              </n-grid>
            </div>
            <div class="level-detail-div">
              ...
              <n-timeline>
                <n-timeline-item v-for="(item, key) in formValue.logList"
                  :type="item.actionWay == 'DONE' ? 'info' : (item.actionWay == 'REFUND' || item.actionWay == 'OVERDUE' ? 'error' : (item.actionWay == 'CANCEL' ? 'warning' : 'default'))"
                  :key="key" :title="item.remark" :time="item.createdAt" />
              </n-timeline>
            </div>
          </div>
              ...
              <n-grid y-gap="25" :cols="2">
                ...
                <n-gi>
                  <div class="level-detail-div-item">
                    ...
                    <div class="level-detail-div-item-content">{{ formValue.orderType == 'CRS' ? '住一自营' : 'INNN' }}
                    </div>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="level-detail-div-item">
                    ...
                    <div class="level-detail-div-item-content">{{ formValue.serviceType == 'PICKUP' ? '接机' : '送机' }}
                    </div>
                  </div>
                </n-gi>
                <n-gi>
          ...
          <div style="padding: 30px 20px" v-else-if="nowTab == 3">
            ...
            <div class="order-table-body">
              <div class="order-table-body-item" v-for="(item, index) in formValue.logList" :key="index">
                ...
                <div style="width: 10%"> <n-image style="margin-left: 10px;margin-top: 5px; height: 45px;" :src="item.images"/></div>
              </div>
            </div>
          </div>
    ...
    <Transfer ref="transferRef" @reloadInfo="reloadInfo" />
...
function dispatchOrder(e) {
  e.preventDefault();
  dispatchRef.value.openModal(formValue.value.id);
}

function transferOrder(e) {
  e.preventDefault();
  transferRef.value.openModal(formValue.value.id);
}
...
// 司机出发
function goOutOrder(e) {
  e.preventDefault();
  formBtnLoading.value = true;
  if (formValue.value.orderStatus != 'WAIT_SERVE') {
    formBtnLoading.value = false;
    message.error('订单状态不正确');
    return false;
  }
  if (formValue.value.dispatchStatus != 'DONE') {
    formBtnLoading.value = false;
    message.error('订单未派单');
    return false;
  }
  if (formValue.value.driverGoTime) {
    formBtnLoading.value = false;
    message.error('司机已出发，请勿重复点击');
    return false;
  }
  dialog.info({
    title: '提示',
...
function printOrder(e) {
  ...
  dialog.info({
    ...
    negativeText: '取消',
    onPositiveClick: () => {
      PrinterCarOrder({ orderId: formValue.value.id }).then((_res) => {
        message.success('操作成功');
      });
    },
    onNegativeClick: () => { },
