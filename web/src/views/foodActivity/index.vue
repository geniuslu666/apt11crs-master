  ...
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">订餐活动表</text>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/foodActivity/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/foodActivity/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
<!--          <n-button type="primary" @click="handleExport" class="min-left-space" v-if="hasPermission(['/foodActivity/export'])">
            <template #icon>
              <n-icon>
                <ExportOutlined />
              </n-icon>
...
import { List, Export, Delete, Status } from '@/api/foodActivity';
...
function addTable() {
  ...
  router.push({ name: 'foodActivityEdit', params: {  } });
}

// 编辑数据
function handleEdit(record: Recordable) {
  // editRef.value.openModal(record);
  router.push({ name: 'foodActivityEdit', params: { id: record.id } });
}
