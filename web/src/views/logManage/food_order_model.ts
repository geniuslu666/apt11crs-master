    ...
    render(row) {
      ...
      return h(
        NTag,
        {
          ...
          type: getOptionTag(options.value.food_log_action_way, row.actionWay),
          bordered: false,
        },
        {
          default: () => getOptionLabel(options.value.food_log_action_way, row.actionWay),
        }
      );
    },
  },
...
export const options = ref({
  food_log_action_way: [] as Option[],
});

// 加载字典数据选项
export function loadOptions() {
  Dicts({
    types: [ 'food_log_action_way'],
  }).then((res) => {
    options.value = res;
