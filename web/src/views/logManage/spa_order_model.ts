    ...
    render(row) {
      ...
      return h(
        NTag,
        {
          ...
          type: getOptionTag(options.value.spa_order_action_way, row.actionWay),
          bordered: false,
        },
        {
          default: () => getOptionLabel(options.value.spa_order_action_way, row.actionWay),
        }
      );
    },
  },
...
export const options = ref({
  ...
  spa_order_action_way: [] as Option[],
});

// 加载字典数据选项
export function loadOptions() {
  Dicts({
    types: [ 'car_log_order_status', 'spa_order_action_way'],
  }).then((res) => {
    options.value = res;
