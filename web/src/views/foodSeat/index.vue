<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="座位管理">
    ...
    <n-card :bordered="false" class="proCard">
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/foodSeat/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/foodSeat/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
import { List, Delete, Status } from '@/api/foodSeat';
