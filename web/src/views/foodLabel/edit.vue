  ...
  <div>
    <n-drawer v-model:show="showModal" :width="dialogWidth" :z-index="99">
      <n-drawer-content closable :header-style="{
        ...
        <template #header>
          <div style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">{{ formValue.id > 0 ? '编辑标签 #' + formValue.id : '添加标签' }}</div>
        </template>
        <template #footer>
        ...
        <n-spin :show="loading" description="请稍候...">
          ...
          >
            <n-grid :cols="1">
              ...
              <n-gi>
                <n-form-item label="标签名称_简体中文" path="content_zh" :show-require-mark="true">
                  <n-input placeholder="请输入标签名称" v-model:value="nameLanguage.zh" @blur="translate('zh',nameLanguage.zh)" />
                </n-form-item>
              </n-gi>
              <n-gi>
                <n-form-item label="标签名称_English" path="content_en">
                  <n-input placeholder="请输入标签名称" v-model:value="nameLanguage.en" @blur="translate('en',nameLanguage.en)" />
                </n-form-item>
              </n-gi>
              <n-gi>
                <n-form-item label="标签名称_日本语" path="content_ja">
                  <n-input placeholder="请输入标签名称" v-model:value="nameLanguage.ja" />
                </n-form-item>
              </n-gi>
              <n-gi>
...
import { Edit, View, MaxSort } from '@/api/foodLabel';
