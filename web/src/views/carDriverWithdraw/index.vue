...
import {WithdrawList, Agree, Transfer} from '@/api/carDriverWithdraw';
    ...
    return h(TableAction as any, {
      ...
      actions: [
        ...
        },
        {
          ...
          auth: ['/carDriverWithdraw/disagree'],
          type: 'error'
        },
        {
          label: '转账',
          ...
          auth: ['/carDriverWithdraw/transfer'],
          type: 'info'
        },
        {
          label: '详情',
          onClick: handleView.bind(null, record),
          auth: ['/carDriverWithdraw/view'],
          type: 'info'
        },
      ],
      // dropDownActions: [
...
const loadDataTable = async (res) => {
  return await WithdrawList({ ...searchFormRef.value?.formModel, ...res });
};
...
function handleDisgree(record: Recordable) {
  disagreeRef.value.openModal(record);
...
function handleTransfer(record: Recordable) {
  ...
  dialog.info({
    ...
    negativeText: '取消',
    onPositiveClick: () => {
      Transfer({id: record.id}).then((_res) => {
        message.success('操作成功');
        reloadTable();
      });
    },
