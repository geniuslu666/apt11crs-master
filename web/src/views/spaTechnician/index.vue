  ...
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">技师管理</text>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" :openChecked="false" :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/spaTechnician/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
...
<!--          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/spaTechnician/delete'])">-->
<!--            <template #icon>-->
<!--              <n-icon>-->
<!--                <DeleteOutlined />-->
<!--              </n-icon>-->
...
import {List,Status,Unbind} from '@/api/spaTechnician';
...
import WorkStatus from '@/views/spaTechnician/work_status.vue';
import {useRouter} from "vue-router";
import Bind from "@/views/spaTechnician/bind.vue";
    ...
    return h(TableAction as any, {
      ...
      actions: [
        ...
        },
        {
          ...
          type: 'success',
          ifShow: () => {
            return record.workStatus != "WORKING";
          },
          auth: ['/spaTechnician/workStatus'],
        },
        {
          label: '绑定用户',
...
// 添加数据
function addTable() {
  router.push({ name: 'spaTechnicianEdit', params: { id: 0 } });
}

// 编辑数据
function handleEdit(record: Recordable) {
  router.push({ name: 'spaTechnicianEdit', params: { id: record.id } });
}
  ...
  let html = h(
    ...
    [
      ...
      h(
        ...
        {
          default: () => '你确定要解绑？解绑后，技师将无法使用技师APP登录，请谨慎操作！',
        }
      )
    ]
  )
