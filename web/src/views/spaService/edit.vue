          ...
          <n-tabs type="line" animated v-model:value="tabValue">
            <n-tab-pane name="base" tab="基础信息">
              <n-grid cols="1 s:1 m:1 l:1 xl:1 2xl:1" responsive="screen">
                ...
                <n-gi span="1" v-if="formValue.channel != 3">
                  <n-form-item label="适用物业" path="propertyType">
                    <n-radio-group v-model:value="formValue.propertyType" name="propertyType">
                      <n-space>
                        <n-radio :value="1">
                          全部物业
                        </n-radio>
                  ...
                  <n-form-item label=" " path="propertyIdsArr" v-if="formValue.propertyType == 2">
                    <n-select
                      placeholder="请选择物业"
            ...
            <n-tab-pane name="price" tab="价格时长">
              <n-table>
                ...
                <tr>
                  ...
                  <th width="150">列表图(160px*160px)</th>
                  <th width="150">详情图(686px*324px)</th>
                  <th width="200">价格</th>
                  <th width="200">时长</th>
                  <th width="100">状态</th>
                  <th width="130">操作</th>
                ...
                <tr :key="index" v-for="(item, index) in priceList">
                  ...
                  <td>
                    <UploadImage :maxNumber="1" v-model:value="item.detailImage" />
                  </td>
                  <td>
                  ...
                  <td>
                    <n-tag v-if="item.status==1" type="success">启用</n-tag>
                    <n-tag v-else type="error">停用</n-tag>
                  </td>
                  <td>
                    <n-button
                      v-if="index > 0"
              ...
              <n-grid cols="1 s:1 m:1 l:1 xl:1 2xl:1" responsive="screen">
                <n-gi span="1">
                  <n-form-item label="简介_简体中文" path="content_zh">
                    <n-input type="textarea" placeholder="请输入简介" v-model:value="contentLanguage.zh" @blur="translate('content','zh',contentLanguage.zh)" style="width: 40%" />
                  </n-form-item>
                  <n-form-item label="简介_日本语" path="content_ja">
                    <n-input type="textarea" placeholder="请输入简介" v-model:value="contentLanguage.ja" @blur="translate('content','ja',contentLanguage.zh)" style="width: 40%" />
                  </n-form-item>
                </n-gi>
              </n-grid>
            </n-tab-pane>
...
function statusPriceItem(index){
  priceList.value[index].status = priceList.value[index].status == 1 ? 2 : 1;
}
...
function confirmForm(e) {
  ...
  formRef.value.validate((errors) => {
    if (!errors) {
      ...
      if(formValue.value.channel != 3 && (formValue.value.propertyType == 2 && formValue.value.propertyIdsArr.length <= 0)){
        formBtnLoading.value = false;
        message.error('请选择物业');
        tabValue.value = 'base';
        return false;
      ...
      if(formValue.value.propertyType == 1){
        formValue.value.propertyIds = '';
      }else{
        formValue.value.propertyIds = formValue.value.channel != 3 ? formValue.value.propertyIdsArr.join(',') : ''
      }

      let priceListArr = priceList.value;
      let priceListErr = false;
      priceListArr.forEach((item) => {
        if(!item.nameLanguage.zh || !item.nameLanguage.en || !item.nameLanguage.ja || !item.image|| !item.detailImage || !item.price || !item.duration){
          priceListErr = true;
          return true;
        }
      })
