        ...
        >
          <n-grid cols="1 s:1 m:1 l:1 xl:1 2xl:1" responsive="screen">
            <n-gi span="1" style="margin-bottom: 25px">
              <n-form-item label="全局维护模式是否开启" path="isOpen">
                <n-switch v-model:value="formValue.isOpen" :unchecked-value="2" :checked-value="1"/>
                <template #feedback>开启维护模式后，用户在APP端不可访问应用，但会显示应用维护说明图文</template>
              </n-form-item>
            </n-gi>
            <n-gi span="1" v-if="formValue.isOpen == 2">
              <n-form-item label="接送机维护模式是否开启" path="pickUpIsOpen">
                <n-switch v-model:value="formValue.pickUpIsOpen" :unchecked-value="2" :checked-value="1"/>
                <template #feedback>开启维护模式后，用户在APP端不可访问应用，但会显示应用维护说明图文</template>
              </n-form-item>
              <n-form-item label=" " path="pickUpServiceType" v-if="formValue.pickUpIsOpen == 2">
                <n-checkbox-group v-model:value="formValue.pickUpServiceTypeArr">
                  <n-space>
                    <n-checkbox
                      value="CRS"
                      label="自营"
            ...
            <n-gi span="1" v-if="formValue.isOpen == 2" style="margin-bottom: 25px">
              <n-form-item label="包车维护模式是否开启" path="busServiceIsOpen">
                <n-switch v-model:value="formValue.busServiceIsOpen" :unchecked-value="2" :checked-value="1"/>
                <template #feedback>开启维护模式后，用户在APP端不可访问应用，但会显示应用维护说明图文</template>
              </n-form-item>
            </n-gi>

            <n-gi span="1" style="margin-bottom: 25px" v-if="formValue.isOpen == 1 || formValue.pickUpIsOpen == 1 || formValue.busServiceIsOpen == 1 || formValue.pickUpServiceTypeArr.length > 0">
            ...
            <n-gi span="1" v-if="(formValue.isOpen == 1 || formValue.pickUpIsOpen == 1 || formValue.busServiceIsOpen == 1 || formValue.pickUpServiceTypeArr.length > 0) && formValue.testType == 1">
            ...
            <n-gi span="1" v-if="(formValue.isOpen == 1 || formValue.pickUpIsOpen == 1 || formValue.busServiceIsOpen == 1 || formValue.pickUpServiceTypeArr.length > 0) && formValue.testType == 2">
              <n-form-item label="指定会员" path="group" style="width: 800px">
...
<script lang="ts" setup>
  ...
  const rules = ref({});
  const group = ref('carmaintenancebasic');
  ...
  function formSubmit() {
    ...
    formRef.value.validate((errors) => {
      if (!errors) {
        if(formValue.value.pickUpIsOpen == 2 && formValue.value.pickUpServiceTypeArr.length <= 0){
          message.error('接送机的服务类型至少选择1个');
          formBtnLoading.value = false;
          return false;
        }

        formValue.value.pickUpServiceType = formValue.value.pickUpServiceTypeArr ? formValue.value.pickUpServiceTypeArr.join(',') : ''
        delete formValue.value.pickUpServiceTypeArr;
      ...
      getConfig({ group: group.value })
        .then((res) => {
          if(res.list){
            ...
            formValue.value.pickUpServiceTypeArr = res.list.pickUpServiceType ? res.list.pickUpServiceType.split(',') : [];
