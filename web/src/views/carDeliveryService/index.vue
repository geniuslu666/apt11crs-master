  ...
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">送机服务</text>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <!--        </template>-->
      <!--      </BasicForm>-->
      <BasicTable  ref="actionRef" :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds"
                   @update:checked-row-keys="handleOnCheckedRow"   @update:sorter="handleUpdateSorter">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/carService/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <!--          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/carService/delete'])">-->
          <!--            <template #icon>-->
          <!--              <n-icon>-->
          <!--                <DeleteOutlined />-->
          <!--              </n-icon>-->
...
import { List, Status, Delete } from '@/api/carService';
...
// 添加数据
function addTable() {
  router.push({ name: 'carDeliveryServiceEdit', params: { id: null } });
}

// 编辑数据
function handleEdit(record: Recordable) {
  router.push({ name: 'carDeliveryServiceEdit', params: { id: record.id } });
}
