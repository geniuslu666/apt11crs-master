  ...
  <div>
    <n-spin :show="loading" description="请稍候...">
      <n-card
        ...
        :title="formValue.id > 0 ? '编辑送机服务 #' + formValue.id : '添加送机服务'"
      >
        <n-form
          ref="formRef"
          :model="formValue"
        ...
        >
          <n-grid cols="1 s:1 m:1 l:1 xl:1 2xl:1" responsive="screen">
            ...
            <n-gi span="1">
              <n-form-item label="出发地" path="startIdsType">
                <n-radio-group v-model:value="formValue.startIdsType" name="startIdsType">
                  <n-space>
                    <n-radio :value="1">
                      全部物业
                    </n-radio>
              ...
              <n-form-item label=" " path="startIdsArr" v-if="formValue.startIdsType == 2">
                <n-select
                  placeholder="请选择出发地"
                  v-model:value="formValue.startIdsArr"
            ...
            <n-gi span="1">
              <n-form-item label="目的机场" path="endIds" :show-require-mark="true">
                <n-select
                  v-model:value="formValue.endIds"
...
function confirmForm(e) {
  ...
  formRef.value.validate((errors) => {
    if (!errors) {
      ...
      if(formValue.value.startIdsType == 2 && formValue.value.startIdsArr.length <= 0){
        formBtnLoading.value = false;
        message.error('请选择出发地');
        return false;
      }

      if(!formValue.value.endIds){
        formBtnLoading.value = false;
        message.error('请选择目的机场');
        return false;
      }


      formValue.value.serviceType = 'Delivery'

      if(formValue.value.startIdsType == 1){
        formValue.value.startIds = '';
      }else{
        formValue.value.startIds = formValue.value.startIdsArr.join(',')
      }
      ...
      Edit(formValue.value).then((_res) => {
        ...
        setTimeout(() => {
          ...
          router.push({ name: 'carDeliveryServiceIndex', params: {  } });
        }, 500);
      }).catch((err) => {
        formBtnLoading.value = false;
      });
...
async function loadCarAddressList(){
  ...
  if(carAddressArrListOrg.list.length > 0){
    carAddressArrListOrg.list.forEach((item) => {
      if(item.typeId == 4){
        if(item.status == 1){
          startCarAddressIds.value.push(item.id)
...
async function load() {
  ...
  formValue.value.endIds = parseInt(detailRes.endIds)
  if(detailRes.startIds){
    let startIdsArr = detailRes.startIds.split(',').map((item) => {
      return parseInt(item)
    });
    formValue.value.startIdsArr = startIdsArr.filter(num => startCarAddressIds.value.includes(num));
    formValue.value.startIdsType = 2
  }else{
    formValue.value.startIdsArr = [];
    formValue.value.startIdsType = 1
  }
}
