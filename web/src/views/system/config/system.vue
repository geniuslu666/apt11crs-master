<template>
  <div>
    <n-grid cols="24 300:1 600:24" :x-gap="15">
      <n-grid-item span="6">
      ...
      <n-grid-item span="18">
        <n-card :bordered="false" size="small" class="proCard" :header-style="{
                    padding: '20px',
                  }"  :content-style="{
                    padding: '0 20px 20px',
                  }">
...
<script lang="ts">
  ...
  const BasicSetting = defineAsyncComponent(() => {
    ...
    GeeTestSetting = defineAsyncComponent(() => {
      return import('./GeeTestSetting.vue');
    }),
  ...
  const typeTabList = [
    ...
    {
      name: '支付配置',
      desc: 'PayCloud(wechat/alipay+)/Paypal/Stripe',
      key: 10,
    },
    // {
    //   name: '微信配置',
