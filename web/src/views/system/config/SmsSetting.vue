  ...
  <div>
    <n-spin :show="show" description="请稍候...">
      ...
      >
        ...
        <n-form-item label="国内驱动" path="smsDrive">
          <n-select
            placeholder="默认发送驱动"
            :options="options.config_internal_sms_drive"
            v-model:value="formValue.internalSmsDrive"
          />
        </n-form-item>
        ...
        <n-tabs type="card" size="small" v-model:value="tabName">
          ...
          <!--一信通-->
          <n-tab-pane name="ums">
            <template #tab> 一信通 </template>
            <n-divider title-placement="left"> 一信通</n-divider>
            <n-form-item label="企业编码" path="smsUmsSpCode">
              <n-input v-model:value="formValue.smsUmsSpCode" placeholder="" />
              <template #feedback>
                企业编号、appKey、appSecret你可以通过 https://js.ums86.com/ 获取
              </template>
            </n-form-item>

            <n-form-item label="AppKey" path="smsUmsAppKey">
              <n-input
                type="password"
                v-model:value="formValue.smsUmsAppKey"
                show-password-on="click"
              >
                <template #password-visible-icon>
                  <n-icon :size="16" :component="GlassesOutline" />
            ...
            <n-form-item label="AppSecret" path="smsUmsAppSecret">
              <n-input
                type="password"
                v-model:value="formValue.smsUmsAppSecret" placeholder=""
                show-password-on="click"
              >
                <template #password-visible-icon>
                  <n-icon :size="16" :component="GlassesOutline" />
            ...
            <n-form-item label="短信模板" path="smsAliYunTemplate">
              <n-dynamic-input
                v-model:value="formValue.smsUmsTemplate"
                preset="pair"
                key-placeholder="事件KEY"
                value-placeholder="模板CODE"
              />
          ...
          <!--twilio-->
          <n-tab-pane name="twilio">
            <template #tab> Twilio </template>
            <n-divider title-placement="left"> Twilio</n-divider>
            <n-form-item label="FromPhone" path="FromPhone">
              <n-input v-model:value="formValue.smsTwilioFromPhone" placeholder="" />
<!--              <template #feedback>
                企业编号、appKey、appSecret你可以通过 https://js.ums86.com/ 获取
              </template>-->
            </n-form-item>

            <n-form-item label="Username" path="smsUmsAppKey">
              <n-input v-model:value="formValue.smsTwilioUsername"/>
            </n-form-item>

            <n-form-item label="Password" path="password">
              <n-input
                type="password"
                v-model:value="formValue.smsTwilioPassword" placeholder=""
                show-password-on="click"
              >
                <template #password-visible-icon>
                  <n-icon :size="16" :component="GlassesOutline" />
            ...
            <n-form-item label="短信模板" path="smsTwilioTemplate">
              <n-dynamic-input
                v-model:value="formValue.smsTwilioTemplate"
                preset="pair"
                key-placeholder="事件KEY"
                value-placeholder="模板CODE"
              />
...
<script lang="ts" setup>
    ...
    new Promise((_resolve, _reject) => {
      getConfig({ group: group.value })
        .then((res) => {
          ...
          res.list.smsUmsTemplate = JSON.parse(res.list.smsUmsTemplate);
          res.list.smsTwilioTemplate = JSON.parse(res.list.smsTwilioTemplate);
  ...
  async function loadOptions() {
    options.value = await Dicts({
      types: ['config_sms_template', 'config_sms_drive', 'config_internal_sms_drive'],
    });
  }
