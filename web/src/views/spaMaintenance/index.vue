  ...
  <div>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/spaMaintenance/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/spaMaintenance/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
...
import { List, Delete } from '@/api/spaMaintenance';
      ...
      actions: [
        {
          ...
          auth: ['/spaMaintenance/edit'],
        },

        {
          label: '删除',
          onClick: handleDelete.bind(null, record),
          auth: ['/spaMaintenance/delete'],
        },
      ],
    });
  },
