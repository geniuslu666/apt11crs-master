  ...
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" :header-style="{
        ...
        <template #header>
          <text style="font-weight: 500;font-size: 18px;color: #3D3D3D;line-height: 25px;">服务商管理</text>
        </template>
    ...
    <n-card :bordered="false" class="proCard" :content-style="{
      ...
      <BasicTable  ref="actionRef" :openChecked="false" :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/spaIsp/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
...
<!--          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/spaIsp/delete'])">-->
<!--            <template #icon>-->
<!--              <n-icon>-->
<!--                <DeleteOutlined />-->
<!--              </n-icon>-->
...
import {List,Status,Unbind} from '@/api/spaIsp';
...
import WorkStatus from '@/views/spaIsp/work_status.vue';
import Bind from "@/views/spaIsp/bind.vue";
import Edit from "@/views/spaIsp/edit.vue";
...
function addTable() {
  ...
  // router.push({ name: 'spaIspEdit', params: { id: 0 } });
}

// 编辑数据
function handleEdit(record: Recordable) {
  editRef.value.openModal(record);
  // router.push({ name: 'spaIspEdit', params: { id: record.id } });
}
...
function handleBind(record: Recordable) {
  bindRef.value.openModal(record.id,record.memberId);
}
