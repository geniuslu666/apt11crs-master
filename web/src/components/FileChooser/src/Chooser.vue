<template>
  <n-grid :x-gap="5" :y-gap="5" :cols="pageGridCols" responsive="screen">
    <n-gi :span="2">
      <n-menu
        ...
        style="width: 100%;max-height: 510px;overflow-y: scroll"
        :default-value="defaultKindValue"
        :on-update:value="handleUpdateKind"
      />
      <div style="display: flex;justify-content: center">
        <n-button type="primary" @click="handleAddKind">
          <template #icon>
              ...
              >
                ...
                >
                  <div @click="handleSelect(item)">
                    <n-image
                      v-if="item.ext === 'jpg' || item.ext === 'jpeg' || item.ext === 'png'"
                      preview-disabled
                      class="image-size"
                      :src="item.midFileUrl"
                      :fallback-src="item.fileUrl"
                      :on-error="errorImg"
                    />
                  ...
                  <template #action style="padding: 5px">
                    <n-space justify="center">
                      <n-button
                        ...
                        size="tiny"
                        type="primary"
                        @click="(item.ext === 'jpg' || item.ext === 'jpeg' || item.ext === 'png') ? handlePreview(item) : handleDown(item)"
                      >
                        <template #icon>
                          <n-icon>
                            <EyeOutlined v-if="item.ext === 'jpg' || item.ext === 'jpeg' || item.ext === 'png'" />
                            <DownloadOutlined v-else />
                          </n-icon>
                        </template>

                        {{ (item.ext === 'jpg' || item.ext === 'jpeg' || item.ext === 'png') ? '预览' : '下载' }}
                      </n-button>
                      <n-button
                        strong
                        secondary
  ...
  <Edit ref="editKindRef" @reloadTable="loadOptions" />
</template>

<script lang="ts" setup>
import {NSpace, NInput, NButton, useMessage, NEllipsis, useDialog, NIcon} from 'naive-ui';
...
import { Delete as DeleteKind } from '@/api/sysAttachmentKind';
  ...
  import Edit from "@/views/sysAttachmentKind/edit.vue";
  ...
  const emit = defineEmits(['saveChange', 'saveChange2']);
...
function handleAddKind() {
  editKindRef.value.openModal(null);
}
  ...
  function handleSelect(item: Attachment) {
    ...
    const filteraa = dataSource.value
      .filter((f) => {
        if (selectList.value?.indexOf(f.fileUrl) != -1) {
          return f;
        }
      })
      .map((m) => {
        ...
        h(
          NButton,
          {
            ...
            onClick: (event: MouseEvent) => {
              ...
              dialog.warning({
                title: '警告',
                content: '你确定要删除 [' + item.label + '] 分类？该操作不可恢复！',
                positiveText: '确定',
                negativeText: '取消',
                onPositiveClick: () => {
                  DeleteKind({ id: item.id }).then((_res) => {
                    message.success('操作成功');
                    loadOptions();
                  });
                },
