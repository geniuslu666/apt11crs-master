...
func (c *ControllerBasics) ChannelFxCenter(ctx context.Context, req *basics.ChannelFxCenterReq) (res *basics.ChannelFxCenterRes, err error) {
	...
	if g.IsEmpty(Channel) {
		// 渠道信息不存在
		err = gerror.New(gi18n.T(ctx, "channel_info_does_not_exist"))
		return
	}
	res.AllBrokerage = Channel.AllBalance
	res.Brokerage = Channel.Balance
	if err = dao.PmsAppStay.Ctx(ctx).Where(dao.PmsAppStay.Columns().Referrer, MemberInfo.Id).Where(dao.PmsAppStay.Columns().OrderStatus, "HAVE_PAID").Page(1, 4).WithAll().OrderDesc(dao.PmsAppStay.Columns().Id).Scan(&PaidAppStay); err != nil && !errors.Is(err, sql.ErrNoRows) {
		// 计算单数据异常
