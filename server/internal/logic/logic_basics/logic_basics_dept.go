...
type sBasicsAdminDept struct{}

func NewBasicsAdminDept() *sBasicsAdminDept {
	return &sBasicsAdminDept{}
}

func init() {
	service.RegisterBasicsAdminDept(NewBasicsAdminDept())
}

// Model 部门ORM模型
func (s *sBasicsAdminDept) Model(ctx context.Context, option ...*handler.Option) *gdb.Model {
	return handler.Model(dao.AdminDept.Ctx(ctx), option...)
}

// Delete 删除
func (s *sBasicsAdminDept) Delete(ctx context.Context, in *input_basics.DeptDeleteInp) (err error) {
...
func (s *sBasicsAdminDept) VerifyUnique(ctx context.Context, in *input_basics.VerifyUniqueInp) (err error) {
	if in.Where == nil {
		return
	}
...
// Edit 修改/新增
func (s *sBasicsAdminDept) Edit(ctx context.Context, in *input_basics.DeptEditInp) (err error) {
	where := g.Map{
		dao.AdminDept.Columns().Name: in.Name,
		dao.AdminDept.Columns().Code: in.Code,
	}
	// 验证唯一性
	err = s.VerifyUnique(ctx, &input_basics.VerifyUniqueInp{Id: in.Id, Where: where})
	if err != nil {
		return
	}
...
// MaxSort 最大排序
func (s *sBasicsAdminDept) MaxSort(ctx context.Context, in *input_basics.DeptMaxSortInp) (res *input_basics.DeptMaxSortModel, err error) {
	if in.Id > 0 {
	...
	if res == nil {
		res = new(input_basics.DeptMaxSortModel)
	}
...
// View 获取指定部门信息
func (s *sBasicsAdminDept) View(ctx context.Context, in *input_basics.DeptViewInp) (res *input_basics.DeptViewModel, err error) {
...
// List 获取列表
func (s *sBasicsAdminDept) List(ctx context.Context, in *input_basics.DeptListInp) (res *input_basics.DeptListModel, err error) {
	res = new(input_basics.DeptListModel)
...
// GetName 获取部门名称
func (s *sBasicsAdminDept) GetName(ctx context.Context, id int64) (name string, err error) {
...
// VerifyDeptId 验证部门ID
func (s *sBasicsAdminDept) VerifyDeptId(ctx context.Context, id int64) (err error) {
	var (
		pid int64 = 0
	...
	// 非超管只获取下级
	if !service.BasicsAdminMember().VerifySuperId(ctx, mb.Id) {
		pid = mb.DeptId
...
func (s *sBasicsAdminDept) Option(ctx context.Context, in *input_basics.DeptOptionInp) (res *input_basics.DeptOptionModel, totalCount int, err error) {
	var (
		mod    = dao.AdminDept.Ctx(ctx)
	...
	res = new(input_basics.DeptOptionModel)
	if models != nil {
...
// treeList 树状列表
func (s *sBasicsAdminDept) treeList(pid int64, nodes []*entity.AdminDept) (list []*input_basics.DeptTree) {
	list = make([]*input_basics.DeptTree, 0)
	for _, v := range nodes {
		if v.Pid == pid {
			item := new(input_basics.DeptTree)
			item.AdminDept = *v
			item.Label = v.Name
			item.Value = v.Id
...
func (s *sBasicsAdminDept) TreeOption(ctx context.Context) (nodes []tree.Node, err error) {
	var models []*input_basics.DeptTreeOption
	if err = s.Model(ctx).Fields(input_basics.DeptTreeOption{}).OrderAsc(dao.AdminDept.Columns().Pid).OrderAsc(dao.AdminDept.Columns().Sort).OrderDesc(dao.AdminDept.Columns().Id).Scan(&models); err != nil {
