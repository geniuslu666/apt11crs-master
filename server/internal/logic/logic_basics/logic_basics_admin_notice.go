...
type sBasicsAdminNotice struct{}

func NewBasicsAdminNotice() *sBasicsAdminNotice {
	return &sBasicsAdminNotice{}
}

func init() {
	service.RegisterBasicsAdminNotice(NewBasicsAdminNotice())
}

// Model Orm模型
func (s *sBasicsAdminNotice) Model(ctx context.Context, option ...*handler.Option) *gdb.Model {
	return handler.Model(dao.AdminNotice.Ctx(ctx), option...)
}

// Delete 删除
func (s *sBasicsAdminNotice) Delete(ctx context.Context, in *input_basics.NoticeDeleteInp) (err error) {
	_, err = s.Model(ctx).Where("id", in.Id).Delete()
	return
}

// Edit 修改/新增
func (s *sBasicsAdminNotice) Edit(ctx context.Context, in *input_basics.NoticeEditInp) (err error) {
...
// Status 更新部门状态
func (s *sBasicsAdminNotice) Status(ctx context.Context, in *input_basics.NoticeStatusInp) (err error) {
	if in.Id <= 0 {
...
// MaxSort 最大排序
func (s *sBasicsAdminNotice) MaxSort(ctx context.Context, in *input_basics.NoticeMaxSortInp) (res *input_basics.NoticeMaxSortModel, err error) {
	if err = dao.AdminNotice.Ctx(ctx).Order("sort desc").Scan(&res); err != nil {
		return
	}

	if res == nil {
		res = new(input_basics.NoticeMaxSortModel)
	}
...
// View 获取指定字典类型信息
func (s *sBasicsAdminNotice) View(ctx context.Context, in *input_basics.NoticeViewInp) (res *input_basics.NoticeViewModel, err error) {
...
// List 获取列表
func (s *sBasicsAdminNotice) List(ctx context.Context, in *input_basics.NoticeListInp) (list []*input_basics.NoticeListModel, totalCount int, err error) {
	...
	for _, v := range list {
		...
		if v.ReceiverGroup == nil || len(v.ReceiverGroup) == 0 {
			v.ReceiverGroup = make([]input_form.AvatarGroup, 0)
		}
...
func (s *sBasicsAdminNotice) PullMessages(ctx context.Context, in *input_basics.PullMessagesInp) (res *input_basics.PullMessagesModel, err error) {
	var memberId = contexts.GetUserId(ctx)
	if memberId <= 0 {
		err = gerror.New("获取用户信息失败！")
		return
	...
	res = new(input_basics.PullMessagesModel)
	unread, err := s.UnreadCount(ctx, &input_basics.NoticeUnreadCountInp{MemberId: memberId, MessageIds: messageIds})
	if err != nil {
		return
	}
...
func (s *sBasicsAdminNotice) UnreadCount(ctx context.Context, in *input_basics.NoticeUnreadCountInp) (res *input_basics.NoticeUnreadCountModel, err error) {
	if in.MemberId <= 0 {
	...
	res = new(input_basics.NoticeUnreadCountModel)
...
func (s *sBasicsAdminNotice) messageIds(ctx context.Context, memberId int64) (ids []int64, err error) {
...
// UpRead 更新已读
func (s *sBasicsAdminNotice) UpRead(ctx context.Context, in *input_basics.NoticeUpReadInp) (err error) {
	var (
...
// ReadAll 已读全部
func (s *sBasicsAdminNotice) ReadAll(ctx context.Context, in *input_basics.NoticeReadAllInp) (err error) {
	var memberId = contexts.GetUserId(ctx)
	if memberId <= 0 {
		err = gerror.New("获取用户信息失败！")
		return
...
func (s *sBasicsAdminNotice) updatedReadClicks(ctx context.Context, noticeId, memberId int64) (err error) {
...
// MessageList 我的消息列表
func (s *sBasicsAdminNotice) MessageList(ctx context.Context, in *input_basics.NoticeMessageListInp) (list []*input_basics.NoticeMessageListModel, totalCount int, err error) {
	var memberId = contexts.GetUserId(ctx)
	if memberId <= 0 {
		err = gerror.New("获取用户信息失败！")
		return
