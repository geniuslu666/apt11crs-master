...
func AppGateway() {
	...
	s.BindMiddleware("/*any", []ghttp.HandlerFunc{
		...
		middleware.TerminalIsAuth, // API鉴权
	}...)
	//s.BindHandler("GET:/api/ws", wsHandler)
	//s.BindMiddlewareDefault(middleware.Ctx)
	//s.BindMiddlewareDefault(middleware.ServerLog)
	//s.BindMiddlewareDefault(Response)
	//s.BindMiddlewareDefault(middleware.CORS)
	//s.BindMiddlewareDefault(middleware.Blacklist)
	//s.BindMiddlewareDefault(middleware.ApiIsAuth)
...
func wsHandler(greq *ghttp.Request) {
	...
	targetURL, err := url.Parse("ws://127.0.0.1:8000/api/ws") // 替换为你的目标服务器地址和路径
	if err != nil {
...
func gatewayHandler(greq *ghttp.Request) {
	...
	if g.IsEmpty(greq.GetBodyString()) {
		...
		//if CheckXXS(ctx, buffParams.String()) {
		...
		encryptedBodyStr := gstr.Trim(string(encryptedBody), "\"")
		if decryptedBody, err = Decrypt(encryptedBodyStr); err != nil {
			err = errors.New("parsing fail")
			goto ERR
		}
		// 对请求体做xss检测是否存在xss风险

		//if CheckXXS(ctx, string(decryptedBody)) {
		//	err = errors.New("illegal request")
		//	goto ERR
		//}
	...
	if strings.HasPrefix(greq.URL.Path, "/api") {
		...
		req.Header = greq.Header
		req.Header.Set("X-Forwarded-For", greq.GetClientIp())
	} else if strings.HasPrefix(greq.URL.Path, "/travelStaff") {
		// 创建新的请求
		if req, err = http.NewRequest(greq.Method, targetPmsURL+greq.URL.Path, buffParams); err != nil {
			goto ERR
		}
...
// CheckXXS 函数用于检查给定的内容是否可能包含跨站脚本（XSS）攻击。
// 它使用bluemonday库来清理和消毒输入内容，并比较清理后的内容与原始内容是否不同来判断是否存在XSS风险。
// 参数content是需要检查的字符串。
// 返回值表示是否检测到潜在的XSS攻击。
func CheckXXS(ctx context.Context, content string) bool {
	safeHTML := xss.FilterXSS(content, xss.XssOption{})
	// 如果清理后的输出与原始输入不同，则可能存在 XSS 攻击
	if safeHTML != content {
		g.Log().Path("logs/APP_GATEWAY").Debug(ctx, "XSS detected:", content)
		g.Log().Path("logs/APP_GATEWAY").Debug(ctx, "XSS cleaned:", safeHTML)
		return true
	}
	return false
}
...
func Response(r *ghttp.Request) {
	...
	r.Response.Header().Set("Content-Security-Policy", "default-src 'self'; script-src 'self'; style-src 'self';frame-src 'none'; object-src 'none';")
	if r.GetError() != nil {
		err = r.GetError()
		goto ERR
	}
