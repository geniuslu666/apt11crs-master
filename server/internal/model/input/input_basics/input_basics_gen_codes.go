...
type GenCodesSelectsModel struct {
	...
	Db        input_form.Selects `json:"db" dc:"数据库选项"`
	Status    input_form.Selects `json:"status" dc:"生成状态"`
	LinkMode  input_form.Selects `json:"linkMode" dc:"关联表方式"`
	BuildMeth input_form.Selects `json:"buildMeth" dc:"生成方式"`
	// 字段表格选项
	FormMode      input_form.Selects `json:"formMode"  dc:"表单组件"`
	FormRole      input_form.Selects `json:"formRole"  dc:"表单验证"`
	DictMode      []*DictTypeTree    `json:"dictMode"  dc:"字典类型"`
	WhereMode     input_form.Selects `json:"whereMode" dc:"查询条件"`
	Addons        input_form.Selects `json:"addons"    dc:"插件选项"`
	TableAlign    input_form.Selects `json:"tableAlign"    dc:"表格排列方式"`
