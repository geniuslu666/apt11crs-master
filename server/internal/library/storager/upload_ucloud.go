...
func (d *UCloudDrive) UploadFile(ctx context.Context, filePath string) (fullPath string, err error) {
	...
	// 流式上传本地小文件
	f2, err := gfile.Open(filePath)
	defer func() { _ = f2.Close() }()
	if err != nil {
		return
	}
	fullPath = GenFullPath(config.UCloudPath, gfile.Ext(f2.Name()))
...
func (d *UCloudDrive) UploadFileName(ctx context.Context, filePath string, fileName string) (fullPath string, err error) {
	if config.UCloudPath == "" {
		err = gerror.New("UCloud存储驱动必须配置存储路径!")
		return
	}
	...
	fullPath = config.UCloudPath + fileName
	err = client.IOPut(f2, fullPath, "")
	return
}
