...
func (d *MinioDrive) UploadFile(ctx context.Context, filePath string) (fullPath string, err error) {
	if config.MinioPath == "" {
	...
	fullPath = GenFullPath(config.MinioPath, gfile.Ext(fileInfo.Name()))
	...
	_, err = client.PutObject(ctx, config.MinioBucket, fullPath, file, fileInfo.Size(), opts)
	return
}
...
func (d *MinioDrive) UploadFileName(ctx context.Context, filePath string, fileName string) (fullPath string, err error) {
	if config.MinioPath == "" {
		err = gerror.New("minio存储驱动必须配置存储路径!")
		return
	}
	...
	fullPath = config.MinioPath + fileName
	if err = s3utils.CheckValidObjectName(fullPath); err != nil {
		return
	}
