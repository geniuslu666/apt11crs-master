...
func (d *QiNiuDrive) UploadFile(ctx context.Context, filePath string) (fullPath string, err error) {
	if config.QiNiuPath == "" {
	...
	fullPath = GenFullPath(config.QiNiuPath, gfile.Ext(f2.Name()))
...
func (d *QiNiuDrive) UploadFileName(ctx context.Context, filePath string, fileName string) (fullPath string, err error) {
	if config.QiNiuPath == "" {
		err = gerror.New("UCloud存储驱动必须配置存储路径!")
		return
	}
	...
	fullPath = config.QiNiuPath + fileName
	err = client.IOPut(f2, fullPath, "")
	return
}
