...
func (d *OssDrive) UploadFile(ctx context.Context, filePath string) (fullPath string, err error) {
	if config.OssPath == "" {
	...
	fullPath = GenFullPath(config.OssPath, gfile.Ext(f2.Name()))
...
func (d *OssDrive) UploadFileName(ctx context.Context, filePath string, fileName string) (fullPath string, err error) {
	if config.OssPath == "" {
		err = gerror.New("OSS存储驱动必须配置存储路径!")
		return
	}
	...
	fullPath = config.OssPath + fileName
	err = bucket.PutObject(fullPath, f2)
	return
}
