...
// UploadFile 上传到腾讯云cos对象存储
func (d *CosDrive) UploadFile(ctx context.Context, filePath string) (fullPath string, err error) {
	var (
		file *os.File
	)
	if config.CosPath == "" {
	...
	if file, err = gfile.Open(filePath); err != nil {
		return
	}
	fullPath = GenFullPath(config.CosPath, gfile.Ext(file.Name()))
	_, err = client.Object.Put(ctx, fullPath, file, nil)
	fullPath = config.CosBucketURL + "/" + fullPath
	return
}
...
// UploadFileName 上传到腾讯云cos对象存储
func (d *CosDrive) UploadFileName(ctx context.Context, filePath string, fileName string) (fullPath string, err error) {
	var (
		file *os.File
	)
	if config.CosPath == "" {
	...
	fullPath = config.CosPath + fileName
	_, err = client.Object.Put(ctx, fullPath, file, nil)
	fullPath = config.CosBucketURL + "/" + fullPath
	return
}
