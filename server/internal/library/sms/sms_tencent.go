...
// SendCode 发送验证码
func (d *TencentDrive) SendCode(ctx context.Context, in *input_basics.SendCodeInp) (err error) {
	/* 必要步骤：
	...
	request.PhoneNumberSet = common.StringPtrs([]string{in.Mobile})
...
func (d *TencentDrive) SendMsg(ctx context.Context, in *input_basics.SendMsgInp) (err error) {
	...
	if len(in.TemplateParams) > 0 {
		for _, item := range in.TemplateParams {
			templateParams = append(templateParams, item)
		}
	}
	...
	/* 下发手机号码，采用 E.164 标准，+[国家或地区码][手机号]
	 * 示例如：+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号*/
	request.PhoneNumberSet = common.StringPtrs(strings.Split(in.Mobile, ","))
	...
	// 打印返回的json字符串
	g.Log().Debugf(ctx, "tencent.sendMsg response:%+v", string(b))
