...
// TerminalLogin 登录
func TerminalLogin(ctx context.Context, user *model.TerminalIdentity) (string, int64, error) {
	var (
		jsonData []byte
		header   []byte
		err      error
	...
	// 登录token
	tokenKey = GetTokenKey("TERMINAL", authKey)
	// 有效时长
...
// TerminalLogout 注销登录
func TerminalLogout(r *ghttp.Request) (err error) {
	var (
		ctx    = r.Context()
...
// TerminalParseLoginUser 解析登录用户信息
func TerminalParseLoginUser(r *ghttp.Request) (user *model.TerminalIdentity, err error) {
	var (
		ctx    = r.Context()
		header = GetAuthorization(r)
	)
	...
	claims, err := TerminalparseToken(ctx, header)
	if err != nil {
	...
	user = claims.TerminalIdentity
	return
}

// TerminalparseToken 解析jwt令牌
func TerminalparseToken(ctx context.Context, header string) (*TerminalClaims, error) {
	var (
		token  []byte
		claims *TerminalClaims
		err    error
