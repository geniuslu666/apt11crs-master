...
func Sorter(in ISorter) func(m *gdb.Model) *gdb.Model {
	return func(m *gdb.Model) *gdb.Model {
		...
		if len(aliases) > 0 {
			var newSorters []*input_form.Sorter
			var removeIndex []int
			for as, table := range aliases {
				...
				var sorter2 []*input_form.Sorter
				for k, sorter := range sorters {
					if gstr.HasPrefix(sorter.ColumnKey, as) {
						sorter2 = append(sorter2, &input_form.Sorter{
...
func removeSorterIndexes(slice []*input_form.Sorter, indexes []int) []*input_form.Sorter {
	removed := make([]*input_form.Sorter, 0)
...
func mappingAndFilterToTableFields(fieldsMap map[string]*gdb.TableField, sorters []*input_form.Sorter) (ser []*input_form.Sorter) {
	if len(fieldsMap) == 0 {
		return
	}
