...
// NormalizeContent 规范化打印内容，将不兼容的字符替换为打印机支持的字符
// 解决芯烨云打印机对某些特殊 Unicode 字符显示为问号的问题
func (p *PrintRequest) NormalizeContent() {
	if p.Content == "" {
		return
	}
	...
	replacements := map[string]string{
		"〜": "～", // 全角波浪号 U+301C -> 全角波浪号 U+FF5E
		"～": "~", // 全角波浪号 U+FF5E -> 半角波浪号（如果上面的也不行）
		"－": "-", // 全角连字符 -> 半角连字符
		"—": "-", // 破折号 -> 半角连字符
		"―": "-", // 水平线 -> 半角连字符
	}
	...
	fmt.Println("NormalizeContent--------------------:", content)
	p.Content = content
}
