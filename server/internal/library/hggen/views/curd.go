...
type CurdOptionsJoin struct {
	...
	Columns     []*input_basics.GenCodesColumnListModel `json:"columns"`
}
...
type OptionsTree struct {
	...
	TitleField  *input_basics.GenCodesColumnListModel
}
...
type FuncDict struct {
	...
	Value       *input_basics.GenCodesColumnListModel
	Label       *input_basics.GenCodesColumnListModel
}
...
type CurdPreviewInput struct {
	In           *input_basics.GenCodesPreviewInp        // 提交参数
	DaoConfig    gendao.CGenDaoInput                     // 生成dao配置
	Config       *model.GenerateConfig                   // 生成配置
	view         *gview.View                             // 视图模板
	content      *input_basics.GenCodesPreviewModel      // 页面代码
	masterFields []*input_basics.GenCodesColumnListModel // 主表字段属性
	pk           *input_basics.GenCodesColumnListModel   // 主键属性
...
func (l *gCurd) initInput(ctx context.Context, in *CurdPreviewInput) (err error) {
	in.content = new(input_basics.GenCodesPreviewModel)
	in.content.Views = make(map[string]*input_basics.GenFile)
...
func initTableField(ctx context.Context, in *CurdPreviewInput) (err error) {
	...
	if len(in.masterFields) == 0 {
		if in.masterFields, err = DoTableColumns(ctx, &input_basics.GenCodesColumnListInp{Name: in.In.DbName, Table: in.In.TableName}, in.DaoConfig); err != nil {
			return
		}
	}
...
// getPkField 获取主键
func getPkField(in *CurdPreviewInput) *input_basics.GenCodesColumnListModel {
	if len(in.masterFields) == 0 {
...
func (l *gCurd) loadView(ctx context.Context, in *CurdPreviewInput) (err error) {
	...
	importService := "APT/internal/service"
	if temp.IsAddon {
		importService = "APT/addons/" + in.In.AddonName + "/service"
	}
...
func (l *gCurd) DoBuild(ctx context.Context, in *CurdBuildInput) (err error) {
	...
	// 处理sql文件
	handleSqlFile := func(vi *input_basics.GenFile) (err error) {
		// 无需生成
...
func (l *gCurd) DoPreview(ctx context.Context, in *CurdPreviewInput) (res *input_basics.GenCodesPreviewModel, err error) {
	// 初始化
...
func (l *gCurd) generateApiContent(ctx context.Context, in *CurdPreviewInput) (err error) {
	var (
		name    = "api.go"
		tplData = g.Map{}
		genFile = new(input_basics.GenFile)
	)
...
func (l *gCurd) generateSqlContent(ctx context.Context, in *CurdPreviewInput) (err error) {
	...
	menus, err := service.BasicsAdminMenu().GetFastList(ctx)
	if err != nil {
		return err
	}
	...
	if genFile.Meth == consts.GenCodesBuildMethCreate {
		...
		hasMenus, err := service.BasicsAdminMenu().Model(ctx).Fields("name").WhereIn("name", menuNames).Array()
		if err != nil {
			return err
		}
