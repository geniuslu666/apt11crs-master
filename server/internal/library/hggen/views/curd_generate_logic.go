...
const (
	LogicWhereComments  = "\n\t// 查询%s\n"
	LogicWhereNoSupport = "\t// 暂不支持生成[ %s ]查询方式，请自行补充此处代码！"
...
func (l *gCurd) generateLogicListWhereEach(buffer *bytes.Buffer, in *CurdPreviewInput, fields []*input_basics.GenCodesColumnListModel, daoName string, alias string) {
	isLink := false
	if alias != "" {
		alias = `"` + alias + `."+`
		isLink = true
